...
scheds =list(map(lambda mv : mv[0] ,mycursor.fetchall()))
mycursor.execute("SELECT id FROM `theater_hall`;")



seats=list(map(lambda x : x[0],mycursor.fetchall()))




mycursor.execute("""SELECT id FROM `movies` WHERE movies.reales_date > "2015-1-1";""")
schedTodayAbove= list(map(lambda mv : mv[0] ,mycursor.fetchall()))
