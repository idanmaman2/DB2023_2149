...
workers = list(map(lambda obj : MaintenaceWorkers(obj[0],obj[1]), zip(workers_id , choices(list(MaintenaceWorkers.Roles) , weights=[6,3,1] , k=WORKERS_NUM))))
print(*workers,sep="\n\n")
print(*list(map(lambda x : tuple(x), workers)))
