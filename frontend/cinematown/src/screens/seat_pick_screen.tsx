...
export function SeatPickScreen() {
    ...
    let getArrayOfSeats = (row: number, col: number, seats:{all : Seat[] , free :Seat[] , broken : Seat[] } ): number[][] => {
        ...
        let array: number[][] = new Array(row + 1).fill(0).map((x)=>new Array(col + 1).fill(0));
        for (var seat of allSeatsExsists) {
            array[seat!.rowseat][seat!.columnseat] = 0;
        }
        for (var seat of freeSeats) {
            array[seat!.rowseat][seat!.columnseat] = 1;
        }
        for (var seat of brokenSeats) {
            array[seat!.rowseat][seat!.columnseat] = 2;
        }
    ...
    return <Show
        ...
        <div class=" flex justify-center items-center">
            ...
            <div class="grid grid-cols-1 w-[50vw] h-[60vh] gap-4">
                <For each={getArrayOfSeats(schedule()!.theater_hall!.sizeRow, schedule()!.theater_hall!.sizeCol, seats()!)}>{
                    (seatsA: any, index: any) => <div class="flex items-start justify-between">
                        <p class="w-[1vw] h-[1vh]">{index().toString().padStart(2, "0")}</p>
                        <For each={seatsA}>{(seat: any, indexj: any) => <button title="I appear on hover" onClick={()=>console.log("cool")}><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-armchair-2" width="50" height="24" viewBox="0 0 10 20" stroke-width="2" stroke="currentColor" fill={seat == 0 ? "#e31919" : seat ==1 ?"#10e348" :"#f78c0a"   } stroke-linecap="round" stroke-linejoin="round"> <path stroke="none" d="M0 0h24v24H0z" fill="none" /> <path d="M5 10v-4a3 3 0 0 1 3 -3h8a3 3 0 0 1 3 3v4" /> <path d="M16 15v-2a3 3 0 1 1 3 3v3h-14v-3a3 3 0 1 1 3 -3v2" /> <path d="M8 12h8" /> <path d="M7 19v2" /> <path d="M17 19v2" /> </svg></button>}</For>
                    </div>
                }
                </For>
