...
export default function MovieBuyForm() {
    ...
    let [movieState, movieSetState] = createResource(theaterIDState, (id) => {
        ...
        return getAvailableMoviesByTheaterAndDate(id.valueOf(), dateState());
    });
    ...
    let [hoursState] = createResource(movieIdState, (id) =>{
        ...
        return getAvailableScheduleBYMoviesAndTheaterAndDate(id.valueOf(),theaterIDState()!.valueOf(), dateState());
    }); 
    ...
    return <div class="w-full h-full">
        ...
        <div class="flex justify-center items-center m-10">
            <div class="grid grid-cols-4 w-full h-full">
                <div class="m-5 border-red-600 border-4 h-[50%] w-[90%] text-3xl flex">
                    ...
                    <input class="w-[80%] h-full" type="date" value={dateState().toJSON().slice(0, 10)} onChange={(e) => {
                ...
                <div class="m-5 border-red-600 border-4 h-[50%] w-[90%] text-3xl flex">
                    ...
                    <select value={theaterIDState() == null ? undefined : theaterIDState()!.valueOf()} id="theater_select" class="w-[80%] h-full" onChange={(e) => {
                        movieSetIdState(null);
                        hourIDSetState(null);
                        theaterIDSetState(new Number(e.currentTarget.value))
                    }}>
                        <option disabled selected value={undefined}>Select : Theater</option>
                        <Show when={theaterState()}>
                            <For each={theaterState()}>{
                                (theater: Theater, index) => <option value={theater.id.valueOf()}>{`${theater.city_name} ${theater.street_name} ` }</option>
                            }
                            </For>
                        </Show>
                    </select>
                ...
                <div class="m-5 border-red-600 border-4 h-[50%] w-[90%] text-3xl flex">
                    ...
                    <select value={movieIdState() == null ? undefined : movieIdState()!.valueOf()} id="movie_select" disabled={movieState() == null || theaterIDState() == null} class="w-[80%] h-full"
                        onChange={(e) => {
                            hourIDSetState(null);
                            movieSetIdState(new Number(e.currentTarget.value))
                        }} >
                ...
                <div class="m-5 border-red-600 border-4 h-[50%] w-[90%] text-3xl flex">
                    ...
                    <select value={hourIDState() == null ? undefined : hourIDState()!.valueOf()} id="hour_select" disabled={movieIdState() == null || theaterIDState() == null} class="w-[80%] h-full" onChange={(e) => hourIDSetState(new Number(e.currentTarget.value))} >
